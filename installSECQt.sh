...
#This script install CQtSoftEdu software
...
if [ $cont -eq 1 ]; then
	...
	echo "Please click CONTINUE to select CQtSoftEdu Installation Folder"
	echo " "
	command -v $_ROOT
	$pathROOTEXE -q
	...
	echo "$pathROOTCONFIG"
	echo " ")
    echo "$infoINS1" | (zenity --text-info --title="CQt SoftEdu install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install')
    if [ $? -eq 1 ]; then
	echo "Exit"
	exit 2
    fi   
    ...
    echo "$infoINS2" | (zenity --text-info --title="CQt SoftEdu install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install')
    if [ $? -eq 1 ]; then
	echo "Exit"
	exit 2
    fi   
...
dirPATH=$(zenity --file-selection   --directory --title="CQt SoftEdu install Choose a Installation Folder" --width=800 --height=400 )

(zenity --info --text="Please click CONTINUE to select Desktop Folder" --title="CQt SoftEdu install choose Desktop Folder" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="CQt SoftEdu install choose a Desktop Folder" --width=800 --height=400)

tarFILE="CQtSoftEdu.tar"
pwd
echo "Installation folder: $dirPATH"
echo "Path to Desktop folder: $dirDESK"
echo "Downloading CQT source files: ... "


echo "wget https://raw.githubusercontent.com/fisinforgh/SECQt/main/$tarFILE -P $dirPATH"
echo "Wait..."

(wget -O $dirPATH/$tarFILE https://raw.githubusercontent.com/fisinforgh/SECQt/main/$tarFILE) | zenity --progress --width=800 --height=400 --auto-close \
										     --title="CQt Install" \
										     --text="Downloading CQtSofEdu... Please wait ..." \
										     --percentage=10
echo " "
...
then
    (echo "Please click CONTINUE to install CQtSoftEdu"
    echo "$tarFILE successfully downloaded!!") | (zenity --text-info --width=800 --height=400  --title="CQt SoftEdu install" \
...
if [ -f "$FILE" ]; then
    rm $FILE
    ($pathROOTCLING -f QTDict.cxx -c QT_SE.h QT_LinkDef.h
     g++ -o SECQt QT_SE.cxx  QTDict.cxx `$pathROOTCONFIG --cflags --glibs`) | zenity --progress --width=800 --height=400 --auto-close \
												    --title="Classical Quantum Tunneling Software Education Install" \
												    --text="Compiling CQt software" \
												    --percentage=10
else
    ($pathROOTCLING -f QTDict.cxx -c QT_SE.h QT_LinkDef.h
     g++ -o SECQt QT_SE.cxx  QTDict.cxx `$pathROOTCONFIG --cflags --glibs`) | zenity --progress --width=800 --height=400 --auto-close \
...
if [ -f "$FILE" ]; then
    ...
    (zenity --info --text="SECQt executable compilation complete $FILE. \nPlease click CONTINUE to setup CQt executable globally and  desktop icon "\
	    --title="SECQt install" --ok-label='CONTINUE' --width=800 --height=400 )
else 
    echo "Something went WRONG. Please check requirements"
    echo "and try again"
    (zenity --info --text="Please check g++ compiler and try it again "\
	    --title="SECQt install ERROR" --ok-label='Exit install' --width=800 --height=400 )
    if [ $? -eq 0 ]; then
	echo "Exit"
	exit 2
    fi
...
echo "Icon=$dirPATH/$nameDIR/Icons/APP.png" >> "$deskFILE"
...
#	    --title="CQTSoftEdu install" --ok-label='Close' --width=800 --height=400  )
exit 0
